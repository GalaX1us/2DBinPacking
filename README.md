...
In this section, we delve into the significance of various parameters in the context of the 2D Bin Packing Problem. Specifically, we explore how these parameters influence solution quality and time complexity.

### Population Size

*The fixed parameters were : kappa = 10, delta = 20, mutation rate = 0.7, crossover rate = 0.2, and number of generations = 200*

![Population Size vs Time Elapsed](img/population_size_perf.png)

The graph depicts the relationship between population size and time elapsed and number of bins. We can see that increasing the population size doesn't significantly impact the time required to find solutions. The `time complexity` appears to be roughly `linear`, indicating that doubling the population size approximately doubles the time taken to find solutions. Quality wise, every population sizes seems to be equivalent.
